...
exports.createPermissions = async (req, res) => {
  try {
    ...
    const filteredPermissions = permissions.filter(p => validModules.includes(p.module_name));

    if (filteredPermissions.length === 0) {
      return res.status(400).json({ message: "No valid module permissions provided" });
    }
