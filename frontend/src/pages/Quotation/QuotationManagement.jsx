  ...
  const fetchQuotations = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:8000/api/quotations/fetch/Processed",
  ...
  const handleSetReminder = async (quotation_number) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8000/api/quotations/${quotation_number}/reminder`,
        { reminder_days: days },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  useEffect(() => {
    const fetchReminders = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          "http://localhost:8000/api/quotations/reminders",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        if (reminders.length > 0) {
          const reminderList = reminders
            .map(
              (r) =>
                `🔔 Quotation ${r.quotation_number} is due today or overdue!`
            )
            .join("\n");

          alert(reminderList); // simple alert; can replace with SweetAlert2 if preferred
        }
      } catch (error) {
                                      ...
                                      if (
                                        ...
                                        try {
                                          ...
                                          const res = await axios.put(
                                            `http://localhost:8000/api/quotations/deactivate_reminder/${q.quotation_number}`,
                                            {},
                                            {
                                              headers: {
