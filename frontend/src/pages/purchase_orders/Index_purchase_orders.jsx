...
const Index_purchase_orders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (res.ok) {
        setOrders(result.data); // set orders array
      } else {
        Swal.fire("Error", result.message || "Failed to fetch orders", "error");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleCancel = async (po_number) => {
    try {
      const res = await fetch(
        `http://localhost:8000/api/purchaseOrder/cancel/${po_number}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      ...
      console.error(err);
      Swal.fire("Error", "Server error while canceling PO", "error");
    }
  };
  ...
  const handleConfirm = async (po_number) => {
    try {
      const res = await fetch(
        `http://localhost:8000/api/purchaseOrder/${po_number}/confirm`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      ...
      if (res.ok) {
        ...
        Swal.fire("Error", data.message || "Failed to confirm PO", "error");
      }
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Server error while confirming PO", "error");
    }
  };
